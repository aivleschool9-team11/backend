...
**AIVLE Book Manager**의 백엔드 서버입니다. 도서 정보의 영속화부터 키워드·의미(시맨틱) 검색, 태그 관리, 검색·클릭 로그 수집까지 — React 프론트엔드가 소비하는 REST API를 제공합니다.

프론트엔드 미니프로젝트 단계에서 사용하던 가상 백엔드(JSON-Server / `db.json`)를 **Spring Boot + JPA + H2로 대체**하고, **AI 표지 저장 API까지 확장**했습니다. 생성형 AI(표지·요약·해시태그·임베딩)는 프론트엔드에서 OpenAI를 직접 호출해 생성하고, **백엔드는 그 결과를 저장·조회·검색**하는 역할을 담당합니다.
...
③ React 내부에서 Data URL로 변환
④ React → Spring Boot        : PATCH /books/{id}/cover (생성된 표지만 저장)
```
...
```
> 클라이언트와는 **DTO로만 주고받고**(엔티티 직접 노출 X), 엔티티는 순수 영속 모델로 유지합니다. 서버 관리 필드(`id`·`createdAt`·`likes` 등)의 외부 주입을 차단하고, 목록 응답을 경량화합니다.
...
### 📌 미션 7 — AI 표지 생성 흐름
프론트가 OpenAI로 표지를 생성하고, 백엔드는 그 결과를 저장. (Base64는 Blob/`multipart`로 전송)
```java
...
// 흐름: React → OpenAI(GPT Image) → React(Data URL) → PATCH /books/{id}/cover → 저장
```
...
#### ① 통합 검색·정렬 & 좋아요
키워드(제목·저자) 검색 + **5종 정렬**(최신순·오래된순·제목순·작가명순·좋아요순) + 태그 필터를 한 메서드에서 처리. 좋아요는 프론트 LocalStorage 찜 상태와 백엔드 전체 카운트를 동기화.
```java
...
- **해결**: 표지를 **거대한 문자열로 보내지 않고, 프론트에서 Base64 → Blob 으로 변환한 뒤 `multipart/form-data`(파일 업로드) 형식으로 전송**하도록 변경. 이미지를 바이너리 형태로 다루어 컬럼 길이 한계 문제를 근본적으로 회피함.
  ```js
  // Frontend: Base64 Data URL → Blob → multipart/form-data 전송
  const blob = await (await fetch(base64DataUrl)).blob();
  const form = new FormData();
  form.append("cover", blob, "cover.png");
  await fetch(`/books/${id}/cover`, { method: "PATCH", body: form });
  ```
- **회고**: "Base64 인코딩 = 매우 긴 문자열"이라는 데이터 특성을 간과한 사례. **대용량 미디어는 텍스트 컬럼에 문자열로 욱여넣기보다, 바이너리(multipart)로 다루는 것**이 적절하다는 점을 학습.
...
## 📦 산출물
- 소스코드(`AI_##조.zip`), 결과보고서(`AI_##조.pptx`), `README.md` 포함 압축 제출
